...
pv_price=111       #Price of PV per kWp
...
pv_list=np.array(range(1,6))*4700
storage_list=np.array(range(1,25))*1.9e3
storage_list_price=storage_list*storage_price/duration
color_list=['r-','g-','b-','y-','m-']
color_id=0

fig,ax=plt.subplots()
refernence_list=[0,680]
...
fig.legend(loc='upper center', bbox_to_anchor=(1.05, 0.8), shadow=True, ncol=1)
plt.show()
...
#
#capacity_factor=solar_data()
#pv_list=np.array(range(13))*2000
#storage_list=np.array(range(5))*1e4
#
#fig,ax=plt.subplots()
#for storage_capacity in storage_list:        
#    total_cost_list=[]
